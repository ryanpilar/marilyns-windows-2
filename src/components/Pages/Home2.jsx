    ...
    const getContentfulContents = async () => {
      try {
        await client.getEntries().then((allEntries) => {
          const aboutContent = allEntries.items.filter(
            (entry) => entry.fields.location === "about"
          );
          const servicesContent = allEntries.items.filter(
            (entry) => entry.fields.location === "services"
          );
          const statsContent = allEntries.items.filter(
            (entry) => entry.fields.location === "statistics"
          );
          const galleryContent = allEntries.items.filter(
            (entry) => entry.fields.location === "gallery"
          );
          const testimonialContent = allEntries.items.filter(
            (entry) => entry.fields.location === "testimonials"
          );
          const background = allEntries.items.filter(
            (entry) => entry.fields.location === "background"
          );
